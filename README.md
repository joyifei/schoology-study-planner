...
- Exports and imports planner data so another Chrome profile or laptop can restore homework done status, course setup, current grades, and GPA chart data.
...
Use `Export Data` in the top bar to download a portable JSON file with homework done status, course setup, current grades, and saved GPA history. On another Chrome profile or laptop, install the extension and use `Import Data` to restore it.
